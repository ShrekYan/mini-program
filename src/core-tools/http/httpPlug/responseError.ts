...
import {IOptions, IResponseData} from "../http";

interface IResponse {
  options: IOptions,
  resp: Taro.request.SuccessCallbackResult<IResponseData<{ data: object }>>
}

type errorToastFnType = (errorMsg: string) => void;

//解决提示方法重复执行导致提示语闪屏现象
const showErrorToast: errorToastFnType = showErrorToastFn();

export default ({options, resp}: IResponse) => {
  ...
  if (autoInteract) {
    ...
    if ((RESPONSE_CODE.SUCCESS !== resp.data.code) && (RESPONSE_CODE.SESSION_TIMEOUT !== resp.data.code)) {
      ...
      if (openCustomError) {
        ...
        if (Object.keys(CUSTOM_ERRORS).length > 0) {
          //从自定义报错信息中获取信息并且提示
          let _tempObj = CUSTOM_ERRORS[respCode];
          if (_tempObj) {
            errorMsg = _tempObj;
          }
        }
