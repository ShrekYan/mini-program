...
Component({
  ...
  lifetimes: {
    attached: function () {
      ...
      //弹出式
      if (res.captchaType == "blockPuzzle" && res.mode == "pop") {
        _self.show();
      }

      _self.setData({
        verifyImgOutHeight: (parseInt(res.imgSize.height) + parseInt(res.vSpace)) + "rpx",
        verifySubBlockWidth: Math.floor(parseInt(res.barSize.width)) + 'rpx',
        verifySubBlockTop: "-" + (parseInt(res.imgSize.height) + parseInt(res.vSpace)) + "rpx",
        verifySubBlockImgTop: parseInt(res.blockImgSpace) + "rpx",
      })
      // _self._uuid();
      _self._refresh();
    },
  ...
  methods: {
    ...
    _blockPuzzleCheck(obj) {
      ...
      //动态计算位移，按照350宽度转化计算
      let movedXValue = Math.round(obj.offsetX);
      movedXValue = 375 / _self.data.windowWidth * movedXValue

      //校验验证码
      _self.data.opt.verifyCheck({
        ticket: _self.data.ticket,
        movedXValue: Math.round(movedXValue)
      }).then(() => {
        _self.setData({
    ...
    _refresh() {
      ...
      //调用获取验证码方法
      _self.data.opt.getCode().then(data => {
        _self.setData({
          originalImageBase64: data.backgroundImage,
          jigsawImageBase64: data.blockImage,
          ticket: data.ticket,
          verifyTipsClass: "",
          leftBarClass: "status-1",
          verifyTipsText: "",
          clickWordTapName: 'blockPuzzle',
          clickWordTapNum: 0,
          clickWordXYList: [],
          clickWordPointList: [
            {left: 0, top: 0, display: 'none'},
